    ...
    foreach ($dataUsuario as $rowUsuario) {
        ...
        if ($dataUsuario2) {
            foreach ($dataUsuario2 as $rowUser) {
                ...
                $pdf->Cell(40, 10, $pdf->encodeString($rowUser['nombre_usuario']), 1, 0, 'L');
                $pdf->Cell(80, 10, $pdf->encodeString($rowUser['correo_usuario']), 1, 0, 'L');
                $pdf->Cell(30, 10, $rowUser['estado'], 1, 1, 'C');
            }
        } else {
            $pdf->SetFont('Arial', '', 10);
